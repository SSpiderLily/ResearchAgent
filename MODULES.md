...
| **输入**     | 本轮用户问题 `question`；可选多轮历史 `history`（仅 `user` / `assistant` 的 `content` 文本） |
| **输出**     | 回答文本 + `references`（论文标题、页码、片段等）                                            |
| **外部依赖** | `embedder`、`vectorstore`、`database`（按 `paper_id` 取标题）、`llm`（DashScope Generation） |
| **配置**     | `RAG_TOP_K`、`CHAT_HISTORY_MAX_MESSAGES`                                                     |

### 7.1 跨论文多轮问答（实现思路）

**目标：** 满足需求文档中「保留对话历史、支持追问与上下文关联」——后端参与建模上下文，而非仅靠前端展示。

1. **请求形态**  
   - `POST /api/chat`  Body：`{ "question": "<本轮用户输入>", "history": [ { "role": "user"|"assistant", "content": "..." }, ... ] }`  
   - `history` 为**本轮之前**的轮次（不含当前 `question`）。前端在追加本轮 user 消息前先序列化已有 `session_state.messages`。

2. **检索仍对齐「当前问句」**  
   - 向量检索仅对当前 `question` 做 `embed_query`，保证每个新问句都会重新召回片段；追问中的指代由 LLM 结合 `history` 与本轮检索结果共同消歧。

3. **拼装发给 LLM 的消息列表**  
   - `system`：系统提示（含「有历史时结合指代，仍优先本轮检索片段」）。  
   - `history`：经 `_normalize_history` 过滤非法 role、空内容，并按 `CHAT_HISTORY_MAX_MESSAGES` **从末尾截断**，控制 token。  
   - 最后一条 `user`：嵌入本轮 Chroma 返回的片段与 `question`（与单轮时相同，仅前面插入多轮历史）。

4. **检索为空时的多轮降级**  
   - 无历史：沿用原提示，引导用户先上传文献。  
   - 有历史：不再直接拒答；用 `system + history + user(说明本轮无检索命中 + 当前问题)` 调用 LLM，**本轮 `references` 为空**，避免捏造页码引用。

5. **后续可演进（未实现）**  
   - **指代消解 / 检索查询改写**：将「那篇」「同上」与历史拼成单次检索 query，提升召回。  
   - **会话持久化**：用 `session_id` + 服务端存储替代前端单次会话。  
...
1. 前端发送本轮 `question` 与此前轮次的 `history` → `POST /api/chat`
2. `rag.ask`：`embed_query(question)` → `query_chunks` → 按需 `get_paper` 补全标题
3. `chat_completion([system, …history, user(片段+问题)])` → 返回 `answer` 与 `references`
